...
{
    public class YenilenmeyenPoliceler:IJob
    {
        ...
        public Task Execute(IJobExecutionContext context)
        {
            ...
            YenilenmeyenPolicelerim();
            return Task.CompletedTask;
        }

        private void YenilenmeyenPolicelerim()
        {
            //GridAyarla();
            var policelerim = Session.PoliceService.GetAll().Where(t => t.PoliceBitisTarih < DateTime.Now).
                Where(durum=>durum.Yenilendimi==0).ToList();
            var RaporPoliceleri = policelerim;
            RaporPoliceleri = policelerim.ToList();
            ...
            form.gridControlPoliceler.DataSource = a.Where(ay => ay.PoliceBitisTarih > son30gün).OrderBy(t => t.PoliceBaslangicTarih).ToList();
            GridDüzenle();
            form.gridViewPoliceler.Columns["Unvan"].GroupIndex = 0;
            ToplamHesapla();
            DosyayaKaydet();
            //Mail Gönderme yap
            if (a.Where(ay => ay.PoliceBitisTarih > son30gün).OrderBy(t => t.PoliceBaslangicTarih).ToList().Count() > 0)
        ...
        private static void MailGonder(string mailAdresleri)
        {
            ...
            ePosta.Subject = "Yenilenmeyen Poliçeler";
            ePosta.IsBodyHtml = true;
            var icerik = File.ReadAllText(Session.TempDosyaYolu + "\\Yenilenmeyen Policeler.html"); ;
            ePosta.Body = icerik;
            ePosta.Attachments.Add(new Attachment(Session.TempDosyaYolu + "\\Yenilenmeyen Policeler.xlsx"));
        ...
        private void DosyayaKaydet()
        {
            ...
            printableComponentLink1.ExportToXlsx(Session.TempDosyaYolu + "\\Yenilenmeyen Policeler.xlsx");
            printableComponentLink1.ExportToHtml(Session.TempDosyaYolu + "\\Yenilenmeyen Policeler.html");
        }
